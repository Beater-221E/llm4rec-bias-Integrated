...
def resolve_distributed_plan(
    ...
    *,
    ...
    if strategy == "single":
        ...
        rank = 0
        nproc = 1
        launch_hint = "python -m llm4rec.cli.main train ..."
        ...
        if strategy.startswith("deepspeed"):
            launch_hint = (
                ...
                f"--use_deepspeed -m llm4rec.cli.main train ..."
            )
        else:
            launch_hint = (
                f"python -m accelerate.commands.launch --num_processes {nproc} "
                f"--multi_gpu -m llm4rec.cli.main train ..."
            )
...
def _rendezvous_dir() -> Path:
    ...
    return Path(tempfile.gettempdir()) / f"llm4rec_rendezvous_{safe}"
